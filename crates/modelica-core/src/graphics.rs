...
pub fn resolve_coordinate_system(icon: &AnnotationCall) -> CoordinateSystem {
...
/// Resolve one graphic call for a non-Icon layer such as Diagram.
pub fn resolve_graphic_call(call: &AnnotationCall) -> (Option<Graphic>, Vec<Diagnostic>) {
    let mut diagnostics = Vec::new();
    let graphic = resolve_graphic(&AnnotationValue::Call(call.clone()), &mut diagnostics);
    (graphic, diagnostics)
}

/// Resolve the `graphics={...}` array of an Icon or Diagram layer.
pub fn resolve_graphics_from_call(call: &AnnotationCall) -> (Vec<Graphic>, Vec<Diagnostic>) {
    let mut diagnostics = Vec::new();
    let graphics = call
        .named("graphics")
        .and_then(AnnotationValue::as_array)
        .into_iter()
        .flatten()
        .filter_map(|value| resolve_graphic(value, &mut diagnostics))
        .collect();
