...
describe("editor Transform", () => {
  ...
  it("translateExtent moves both points", () => {
    const r = translateExtent(
      { p1: { x: -80, y: -40 }, p2: { x: 80, y: 40 } },
      20,
      10,
    );
  ...
  it("applyTransform moves extent for Rectangle", () => {
    const rect: RectangleDto = {
      type: "Rectangle",
      extent: { p1: { x: -50, y: -30 }, p2: { x: 50, y: 30 } },
    };
    const t: GraphicTransform = {
      translate: { x: 20, y: 10 },
      scale: { x: 1, y: 1 },
      rotate: 0,
    };
  ...
  it("boundsOf computes box for extent", () => {
    const rect: RectangleDto = {
      type: "Rectangle",
      extent: { p1: { x: -100, y: -50 }, p2: { x: 100, y: 50 } },
    };
    expect(boundsOf(rect)).toEqual({
      x: -100,
      y: -50,
      width: 200,
      height: 100,
